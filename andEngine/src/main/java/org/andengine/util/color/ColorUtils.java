...
//	private static final int INT_BITS_TO_FLOAT_MASK = 0xFEFFFFFF; // To avoid producing NaN in the int->float conversion.
    ...
    public static float[] HexToOpenGL(String hexColor)
    {
        float[] GLVal = new float[3];
        ...
        GLVal[0] = Integer.parseInt(hexColor.substring(0,2),16) / 255.0f;
        GLVal[1] = Integer.parseInt(hexColor.substring(2,4),16) / 255.0f;
        GLVal[2] = Integer.parseInt(hexColor.substring(4,6),16) / 255.0f;
